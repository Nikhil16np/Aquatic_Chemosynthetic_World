...
def update_graph(plot_type):
    if plot_type == 'histogram':
        ...
        fig = px.imshow(corr_matrix, text_auto=True, color_continuous_scale='coolwarm', title='Correlation Heatmap')
    elif plot_type == 'sigmoid':
        # Sigmoid function plot for Temperature vs Ph
        Temperature = np.linspace(-6, 6, num=1000)
        sigmoid_values = 1 / (1 + np.exp(-Temperature))
        fig = go.Figure(go.Scatter(x=Temperature, y=sigmoid_values, mode='lines'))
        fig.update_layout(title="Sigmoid Function", xaxis_title="Temperature", yaxis_title="Ph")
    elif plot_type == 'countplot':
        # Countplot for Longitude
        fig = px.histogram(data, x='Longitude', title='Count of Longitudes')
